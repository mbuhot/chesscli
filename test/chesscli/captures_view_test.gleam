...
pub fn initial_board_no_captures_test() {
  let commands = captures_view.render(board.initial(), True, 0, 13, 4, None, None, None)
  let result = virtual_terminal.render_to_string(commands, 40, 14)
  // Row 0 and row 13 should be empty (no captures)
  assert result
    == "\n\n\n\n\n\n\n\n\n\n\n\n\n"
}
...
pub fn white_captures_black_pawn_from_white_perspective_test() {
  ...
  let commands = captures_view.render(b, True, 0, 13, 4, None, None, None)
  let result = virtual_terminal.render_to_string(commands, 40, 14)
  // Top row (0) = black's captures (empty), bottom row (13) = white's captures
  // Split into rows
  let rows = string_split(result, "\n")
  let assert Ok(top_row) = list_at(rows, 0)
  let assert Ok(bottom_row) = list_at(rows, 13)
...
pub fn white_captures_from_black_perspective_test() {
  ...
  let b = board.remove(board.initial(), square.a7)
  let commands = captures_view.render(b, False, 0, 13, 4, None, None, None)
  let result = virtual_terminal.render_to_string(commands, 40, 14)
