...
pub fn format_move_list_empty_game_test() {
  let g = game.new()
  let entries = info_panel.format_move_list(g, option.None)
  assert entries == []
}
...
pub fn format_move_lines_pairs_test() {
  ...
  let g = game.from_pgn(pgn_game)
  let g = game.goto_end(g)
  let lines = info_panel.format_move_lines(g, option.None)
...
pub fn render_produces_commands_test() {
  ...
  let g = game.from_pgn(pgn_game)
  let g = game.goto_end(g)
  let commands = info_panel.render(g, 32, 1, 10, option.None)
  assert commands != []
}
...
pub fn render_long_game_limits_output_test() {
  ...
  let commands = info_panel.render(g, 32, 1, 8, option.None)
...
pub fn format_with_analysis_propagates_classification_test() {
  ...
  let entries = info_panel.format_move_list(g, option.Some(ga))
...
pub fn render_with_analysis_includes_color_commands_test() {
  ...
  let commands = info_panel.render(g, 32, 1, 10, option.Some(ga))
